...
multi() ->
	...
	Conn2B = Conn2#nkport{meta=maps:remove(user, Conn2#nkport.meta)},
    [Listen1, Conn2B] = nkpacket:get_all(dom1),
    [
    	Listen2,
    	#nkport{
    		transp = ws,
	...
	{error, closed} = nkpacket:send(dom5, "<test://127.0.0.1:"++P1S++"/dom3;transport=ws>", msg4),
	{error, closed} = nkpacket:send(dom5, "<test://127.0.0.1:"++P1S++"/dom3;transport=ws>;ws_proto=proto1", msg4),

	{ok, Conn3} = nkpacket:send(dom5, "<test://127.0.0.1:"++P1S++"/dom3;transport=ws>;ws_proto=proto1;host=localhost", msg4),
